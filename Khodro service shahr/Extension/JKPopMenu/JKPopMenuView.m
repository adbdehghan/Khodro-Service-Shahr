...
//        maskView.backgroundColor =[UIColor clearColor]; //[RGB_COLOR(210, 210, 210) colorWithAlphaComponent:0.6];
...
- (void)setupSubviews
{
    ...
    if (count%2 == 0) {
//        offset = (JK_SCREEN_WIDTH - PopMenuItemWidth)/4;
        
        offset = ((JK_SCREEN_WIDTH/PopMenuItemWidth -2)/3)*PopMenuItemWidth;

        // 3 的情况
        NSInteger verticalNum = self.menuItems.count/2;
        CGFloat yStart = (JK_SCREEN_HEIGHT - verticalNum*(PopMenuItemHeight + y_Offset/2))/2 +38;
        firstItemY = yStart;
        NSInteger index = 0;
        for (JKPopMenuItem *item in self.menuItems) {
            NSInteger y_index = index/2;
