...
- (instancetype)initWithFrame:(CGRect)frame delegate:(id<LCBannerViewDelegate>)delegate imageName:(UIImage *)imageName count:(NSInteger)count timerInterval:(NSInteger)timeInterval currentPageIndicatorTintColor:(UIColor *)currentPageIndicatorTintColor pageIndicatorTintColor:(UIColor *)pageIndicatorTintColor {
...
- (void)setupMainView {
    ...
    [self addSubview:({
        ...
        for (int i = 0; i < self.count + 2; i++) {
            ...
            if (self.imageName != nil) {    // from local
                
                UIImage *image = self.imageName;
                if (!image) {
    ...
    [self addSubview:({
        ...
        pageControl.currentPageIndicatorTintColor = self.currentPageIndicatorTintColor ?: [UIColor greenColor];
