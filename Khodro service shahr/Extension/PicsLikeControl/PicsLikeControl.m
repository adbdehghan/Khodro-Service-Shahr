...
{
    if ([self.delegate respondsToSelector:@selector(controlTappedAtIndex:Sender:)])
    {
        [self.delegate controlTappedAtIndex:self.currentImageIndex Sender:self];
        _flipped = YES;
...
-(void)ManualTap
{
   // _flipped = YES;
    ...
    UIView *frontView = self.couple[1];
    [self bringViewToFrontAnimate:frontView];
...
-(void)Dislike
{
    ...
    UIView *frontView = self.couple[0];
    [self bringViewToFrontAnimate:frontView];
